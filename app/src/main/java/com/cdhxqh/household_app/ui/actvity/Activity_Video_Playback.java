...
public class Activity_Video_Playback extends BaseActivity implements View.OnClickListener, SurfaceHolder.Callback,
        Handler.Callback, TimeBarHorizontalScrollView.TimeScrollBarScrollListener, SecureValidate.SecureValidateListener, OpenYSService.OpenYSServiceListener {
    private static final String TAG = "Activity_Video_Playback";
    /**
     * 返回按钮*
     */
...
//        titleView.setText(getString(R.string.video_playback_title));
        ...
        if (mAlarmStartTime == null) {
            titleView.setText(Utils.date2String(mStartTime.getTime()));
    ...
    private void onRecordBtnClick() {
        ...
        if (mRemotePlayBackMgr != null) {
            ...
            mRemotePlayBackHelper.startRecordTask(mRemotePlayBackMgr, Activity_Video_Playback.this.getResources(), R.drawable.video_file_watermark);
        }
    }
    ...
    private void setPlayScaleUI(float scale, CustomRect oRect, CustomRect curRect) {
        if (scale == 1) {
            ...
            try {
                mRemotePlayBackMgr.setDisplayRegion(false, null, null);
            } catch (BaseException e) {
            ...
            if (mPlayScale == scale) {
                try {
                    mRemotePlayBackMgr.setDisplayRegion(true, oRect, curRect);
                } catch (BaseException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                }
